...
export default function Main() {
  ...
  useEffect(() => {
    ...
    if (sortOption === 'name_asc') {
      temp.sort((a, b) => a.workspace_name.localeCompare(b.workspace_name));
    } else if (sortOption === 'name_desc') {
      temp.sort((a, b) => b.workspace_name.localeCompare(a.workspace_name));
    }
  ...
  const toggleSelectAll = () => {
    if (selectAll) {
      ...
      const allIds = filteredWorkspaces.map(ws => ws.id);
    ...
    for (let i = 1; i <= totalPages; i++) {
      pages.push(
        <button
          ...
          className={`w-8 h-8 rounded-full ${currentPage === i ? 'bg-blue-500 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}
        >
          {i}
        </button>
      );
  ...
  return (
    <div className="max-w-[1102px] mx-auto p-5 bg-white border border-[rgba(131,131,131,0.14)] rounded-2xl shadow">
      {/* Top Controls */}
      {/* Top Controls */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {currentWorkspaces.map(ws => (
          <div
            ...
            className={`border rounded-xl transition-shadow relative ${selectedIds.includes(ws.id) ? 'ring-2 ring-blue-400' : ''}`}
            onClick={() => {
              if (selectAll) {
                setSelectedIds(prev => prev.includes(ws.id) ? prev.filter(id => id !== ws.id) : [...prev, ws.id]);
              }
            }}
          >
